...
int main(int argc, char **argv)
{
  ...
  // === Yeni: Port takılana kadar bekle ===
  struct stat buffer;
  while (stat(portName.c_str(), &buffer) != 0) {
    ROS_WARN_STREAM(portName << " portu bulunamadı. Takılana kadar bekleniyor...");
    sleep(1);
  }
  ROS_INFO_STREAM(portName << " bulundu.");
